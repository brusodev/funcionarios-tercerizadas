...
def main():
    ...
    # Salvar backup do arquivo atual
    index_file = SRC_DIR / "index.html"
    if index_file.exists():
        backup_file = BACKUP_DIR / f"index_backup_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.html"
        import shutil
        shutil.copy2(index_file, backup_file)
        print(f"Backup criado: {backup_file}")

    # Salvar o HTML gerado como backup também
    output_file = BACKUP_DIR / "sections_generated_latest.html"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(all_html)

    print(f"\nHTML gerado salvo em: {output_file}")
    print("Para atualizar o index.html, execute: python scripts/update_index.py")
