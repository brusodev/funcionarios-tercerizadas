...
def extract_recintos_and_unidades(text, pdf_name):
    ...
    Fixed pattern to capture lowercase letters in city names
    """
    ...
    while i < len(lines):
        ...
        # Pattern 2: Nested Recinto - including Araraquara (with lowercase letters allowed)
        # Format: "16 - 810900 Regional DMA - Araraquara/SP"
        recinto_match = re.search(r'(\d+)\s+-\s+(\d+)\s+Regional\s+DMA\s+-\s+(.+?)/[A-Z]{2}', line)
        if recinto_match:
...
def main():
    ...
    print("=" * 90)
    print("EXTRATOR DE APARELHOS POR RECINTO/UNIDADE - V7")
    print("Pattern FINAL com suporte correto para Araraquara e recintos aninhados")
    print("=" * 90)
    print()
    ...
    if total_aparelhos == 32015:
        ...
        print(f"ATENCAO: Total de {total_aparelhos} aparelhos (esperado 32.015, diferenca de {32015 - total_aparelhos})")
    print()
