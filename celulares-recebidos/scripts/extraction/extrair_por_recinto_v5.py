...
def extract_recintos_and_unidades(text, pdf_name):
    ...
    while i < len(lines):
        ...
        # Pattern 2: Nested Recinto (16 - 810900 Regional DMA - Araraquara/SP)
        # This is the critical pattern for Araraquara and similar nested recintos
        recinto_match = re.match(r'(\d+)\s*-\s*(\d{6})\s+Regional\s+DMA\s*-\s*([A-ZÁ-Ú\s/]+)/[A-Z]{2}', line)
        if recinto_match:
        ...
        # The line should start with digit, comma, digit (e.g., "1,00 un SMARTPHONE...")
        ...
        if device_match and (current_unidade or current_recinto):
            try:
                ...
                # Extract value from line (look for R$ followed by value)
                value_match = re.search(r'R\$\s*([\d.]+,\d+)', line)
                valor = 0.0
                if value_match:
                    valor_str = value_match.group(1)
                    # Convert Brazilian format (1.234,56) to float (1234.56)
...
def main():
    ...
    print("=" * 90)
    print("EXTRATOR DE APARELHOS POR RECINTO/UNIDADE - V5")
    print("Incluindo recintos aninhados com pattern melhorado para Araraquara")
    print("=" * 90)
    print()
    ...
    # Save to JSON
    json_path = os.path.join(folder_path, 'json', 'aparelhos_por_recinto_v5.json')
