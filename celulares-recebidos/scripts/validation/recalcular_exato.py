...
# As 4 linhas EXATAS a remover do PDF 0735 (as 4 "consoluidações")
# Vão ser removidas EXATAMENTE por linha
linhas_id_remover = {352, 354, 356, 824}  # índices das linhas a remover no PDF 0735
...
print("=== RECONTAGEM COM REMOÇÃO DE DUPLICATAS (EXATAS) ===\n")
...
for pdf_file in pdf_files:
    try:
        ...
        for idx, linha in enumerate(lines):
            # SE for PDF 0735 E for uma das 4 linhas a remover, pule
            if 'ADM 0800100_0735_2025' in pdf_file and idx in linhas_id_remover:
                print(f"   ⚠️  REMOVENDO linha {idx}: {linha[:60]}...")
                continue
        ...
        # Calcular total de aparelhos DESTE PDF (sem as duplicatas)
...
print("\n" + "="*60)
print("=== TOTAIS POR RECINTO (CORRIGIDO) ===")
print("="*60 + "\n")
...
# Salvar JSON
with open('aparelhos_por_recinto_CORRIGIDO_FINAL.json', 'w', encoding='utf-8') as f:
    json.dump({
        'total': {
            'aparelhos': total_geral_aparelhos,
            'valor': f"R$ {total_geral_valor:,.2f}"
        },
        'por_recinto': {k: {
...
print("\n✅ Arquivo salvo: aparelhos_por_recinto_CORRIGIDO_FINAL.json")
